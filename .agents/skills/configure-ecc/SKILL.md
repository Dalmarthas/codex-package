...
Options:
  - "User-level (~/.codex/)" — "Applies to all your Claude Code projects"
...
- If a skill references `~/.codex/config.toml` — this is usually fine (settings are always user-level)
- If a skill references `~/.agents/skills/` or `~/.codex/rules/` — this may be broken if installed only at project level
